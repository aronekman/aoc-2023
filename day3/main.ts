...
input.split('\n').forEach((row, y) => {
  ...
  while (match) {
    numbers.push({ value: parseInt(match[0]), coord: { x: index + (match.index ?? 0), y } });
...
const part2Result = gears.reduce((prev, gear) => {
  const neighbours = numbers.filter(
    (number) =>
      number.coord.y <= gear.y + 1 &&
      number.coord.y >= gear.y - 1 &&
      number.coord.x + number.value.toString().length >= gear.x &&
      number.coord.x <= gear.x + 1
  );
  return neighbours.length == 2 ? prev + neighbours[0].value * neighbours[1].value : prev;
}, 0);
